...
var svg1 = d3.select("#horizontal_bar_chart1")
.append("svg")
...
var yScale1 = d3.scaleBand().range([height1, 0]).padding(0.25)
...
svg1.append("g")
  ...
  .call(d3.axisBottom(xScale1).tickSize(-height1*1.3).ticks())
  .select('.domain').remove();

//adding y axis
svg1.append("g")
  .call(d3.axisLeft(yScale1).tickSize(-width1*1.3).ticks())
  .select('.domain').remove();
...
//defining tooltip
var Tooltip = d3.select("#horizontal_bar_chart1")
.append('div')
.style("opacity", 0.9)
.attr("class", "tooltip")
