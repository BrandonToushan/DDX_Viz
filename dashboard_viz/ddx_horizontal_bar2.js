...
.attr("y",0)
.attr("height", height1)
.attr("width", width1 + 10)
.style("fill", "#EBEBEB")

//reading the data (.csv)
var dataset2 = d3.csv("https://raw.githubusercontent.com/BrandonToushan/Datasets/master/scores2.csv",function(error,data){
  if (error) {
    throw error;
  }
...
var xScale2 = d3.scaleLinear().range([0, width1]);

//defining discrete band scale for y axis
var yScale2 = d3.scaleBand().range([height1, 0]).padding(0.25)

//providing domain values to x and y scales
xScale2.domain([0, 100]);
yScale2.domain(data.map(function(d){ return d.class; }));

//adding x axis
svg.append("g")
  .attr("transform", "translate(0," + height1 + ")")
  .call(d3.axisBottom(xScale2).tickSize(-height1*1.3).ticks())
  .select('.domain').remove();

//adding y axis
svg.append("g")
  .call(d3.axisLeft(yScale2).tickSize(-width1*1.3).ticks())
  .select('.domain').remove();
...
//defining tooltip
var Tooltip = d3.select("#horizontal_bar_chart2")
.append('div')
.style("opacity", 0.9)
.attr("class", "tooltip")
...
//adding bars
  ...
  .attr('width',function(d) {return xScale2(d.value2); })
  .attr('y', function(d) { return yScale2(d.class); })
